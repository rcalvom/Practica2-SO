...
long insertElement(struct HashTable *table, char *nombre, short AddLast){
    ...
    long id;
    struct Node *nodo;
    do{ nodo = malloc(sizeof(struct Node)); } while(nodo == NULL);
    char *respaldoNombre;
    do{ respaldoNombre = malloc(SIZE); } while(respaldoNombre == NULL);
    bzero(nodo, sizeof(struct Node));
    bzero(respaldoNombre, SIZE);
    ...
    if(index < 0 || TAMANOTABLA < index){
        free(nodo);
        return -1;
    }
    
    nodo->next = NULL;
    struct LinkedList *l = &table->list[index%NUMOFLISTS];
    struct Node *FN = l->FirstNode;
    if(FN == NULL){
        id = index;
        l->FirstNode = nodo;
        l->LastNode = nodo;
        ...
        if(AddLast == 0){
            ...
            id = l->LastNode->id + TAMANOTABLA;
            asm("AddLast:");
            l->LastNode->next = nodo;
            l->LastNode = nodo;
        }
        l->Size++;
    }
